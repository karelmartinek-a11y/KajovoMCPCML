  ...
  try {
    ...
    const evidence = await db.query(
      `select
         ...
         not exists(select 1 from audit_event where object_id=$2 and (coalesce(before_json::text,'')||coalesce(after_json::text,'')) like '%'||$3||'%') as audit_access_token_free,
         not exists(select 1 from runtime_log_event where server_id=$2 and fields::text like '%'||$3||'%') as log_access_token_free,
         not exists(select 1 from audit_event where object_id=$2 and (coalesce(before_json::text,'')||coalesce(after_json::text,'')) like '%'||$4||'%') as audit_kaja_secret_free,

...
describe.skipIf(!enabled)("external component circuit persistence", () => {
  beforeAll(async () => {
    ...
    await db.query(`insert into component(id,kcml_number,code,hostname,display_name,category,registration_type,component_role,lifecycle_state,activation_state,operational_state,monitoring_state,enabled,ingress_enabled,pulse_enabled,egress_enabled,release_version) values ($1,92001,'KCML92001','kcml92001.component.test','Circuit source','AI_CLIENT','GENERIC_COMPONENT','CLIENT','ACTIVE','ACTIVE','HEALTHY','HEALTHY',true,true,true,true,$2) on conflict (id) do update set lifecycle_state='ACTIVE',activation_state='ACTIVE',operational_state='HEALTHY',monitoring_state='HEALTHY',enabled=true,ingress_enabled=true,pulse_enabled=true,egress_enabled=true,deregistered_at=null`, [componentId, KCML_RELEASE.catalogVersion]);
  ...
  it("rejects an open circuit before permission lookup and network dispatch", async () => {
    await expect(dispatchExternalComponentCall(db, { sourceComponentId: componentId, targetKey: "circuit-test", routePath: "/mcp", scopeName: "component.outbound.pulse", payload: {}, correlationId: "92000000-0000-4000-8000-000000000003", hmacKey: Buffer.alloc(32, 1), keyId: "test" })).rejects.toMatchObject({ message: "external_gateway_circuit_open", statusCode: 503 });
  });
  ...
  it("does not reserve a half-open probe for a denied request after cooldown", async () => {
    ...
    await expect(dispatchExternalComponentCall(db, { sourceComponentId: componentId, targetKey: "circuit-test", routePath: "/mcp", scopeName: "component.outbound.pulse", payload: {}, correlationId: "92000000-0000-4000-8000-000000000004", hmacKey: Buffer.alloc(32, 1), keyId: "test" })).rejects.toMatchObject({ message: "external_route_denied", statusCode: 403 });

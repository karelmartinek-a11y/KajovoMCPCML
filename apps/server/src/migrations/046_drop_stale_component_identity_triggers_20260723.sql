drop trigger if exists legacy_mcp_server_component_adapter_trigger on mcp_server;
drop trigger if exists legacy_managed_service_component_adapter_trigger on managed_service;
drop function if exists legacy_mcp_server_component_adapter();
drop function if exists legacy_managed_service_component_adapter();
...
begin
  ...
  values (
    ...
    coalesce(new.release_version, '2026.07.23'),
...
begin
  if not exists (
    select 1
      from pg_trigger
     where tgname = 'mcp_server_component_identity_bridge'
       and tgrelid = 'public.mcp_server'::regclass
       and not tgisinternal
  ) then
    raise exception 'mcp_server_component_identity_bridge_missing';
  end if;

  if not exists (
    select 1
      from pg_trigger
     where tgname = 'managed_service_component_identity_bridge'
       and tgrelid = 'public.managed_service'::regclass
       and not tgisinternal
  ) then
    raise exception 'managed_service_component_identity_bridge_missing';
  end if;
end $$;

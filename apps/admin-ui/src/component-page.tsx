...
function ComponentDetail({ component, role, busy, onClose, onToggle, onLifecycle, onPermission, onCredentialRevoke, onCredentialRotate }: {
  ...
  return <Modal title={`${component.code} · ${component.displayName}`} onClose={onClose} className="component-detail-modal">
    ...
    <section className="component-detail-section"><h3>Component credentials</h3>{component.credentials?.length ? <div className="table-scroll"><table><thead><tr><th>Client ID</th><th>Fingerprint</th><th>Stav</th><th>Naposledy použito</th>{role !== "AUDITOR" ? <th>Akce</th> : null}</tr></thead><tbody>{component.credentials.map((credential) => <tr key={credential.id}><td>{credential.public_id}</td><td><code>{credential.secret_fingerprint}</code></td><td>{credential.status}</td><td>{formatDate(credential.last_used_at)}</td>{role !== "AUDITOR" ? <td><div className="inline-actions"><button className="secondary compact-button" disabled={busy || credential.status !== "ACTIVE"} onClick={() => confirmed("Rotovat credential? Současný credential a jeho access tokeny budou explicitně revokovány.", () => { void onCredentialRotate(component, credential.id).then(setCredentialSecret); })}>Rotovat</button><button className="danger-button compact-button" disabled={busy || credential.status !== "ACTIVE"} onClick={() => confirmed("Nevratně revokovat tento credential a jeho access tokeny?", () => { void onCredentialRevoke(component, credential.id); })}>Revokovat</button></div></td> : null}</tr>)}</tbody></table></div> : <p>Žádné component credential. Tajné hodnoty se v UI nikdy nezobrazují.</p>}</section>
    {credentialSecret ? <section className="component-secret-reveal" role="status"><h3>Nový credential – zobrazí se pouze nyní</h3><dl><dt>Client ID</dt><dd><code>{credentialSecret.clientId}</code></dd><dt>Client secret</dt><dd><code>{credentialSecret.clientSecret}</code></dd><dt>Fingerprint</dt><dd><code>{credentialSecret.fingerprint}</code></dd></dl><button className="secondary" onClick={() => setCredentialSecret(null)}>Rozumím, skrýt secret</button></section> : null}
...
export function ComponentCatalogPage({ components, role, onRefresh, onLoadDetail, onToggle, onLifecycle, onPermission, onCredentialRevoke, onCredentialRotate }: {
  ...
  return <>
    ...
    <section className="panel component-catalog-panel">
      ...
      {filtered.length ? <div className="table-scroll"><table><thead><tr><th>Komponenta</th><th>Kategorie / role</th><th>Lifecycle</th><th>Technický stav</th><th>Verze / capabilities</th><th>Oprávnění / credentialy</th><th>Audit</th><th>Akce</th></tr></thead><tbody>{filtered.map((component) => <tr key={component.id}><td><strong>{component.displayName}</strong><span className="cell-subtitle">{component.code} · {component.hostname}</span></td><td>{component.category}<span className="cell-subtitle">{component.role}</span></td><td><span className={`badge ${tone(component.lifecycleState)}`}>{component.lifecycleState}</span><span className="cell-subtitle">{component.activationState}</span></td><td><span className={`badge ${tone(component.operationalState)}`}>{component.operationalState}</span><span className="cell-subtitle">Monitoring: {component.monitoringState}</span></td><td>{component.revision ?? "-"}<span className="cell-subtitle">{component.capabilities.length} capabilities</span></td><td>{component.permissionCount} / {component.credentialCount}<span className="cell-subtitle"><KeyRound size={12} /> bezpečné fingerprinty</span></td><td><span className={`badge ${tone(component.audit.gapState)}`}>{component.audit.gapState}</span><span className="cell-subtitle">ACK {component.audit.highestAcknowledgedSequence}</span></td><td><IconButton label={`Detail komponenty ${component.displayName}`} onClick={() => { void open(component); }}><MoreHorizontal size={17} /></IconButton></td></tr>)}</tbody></table></div> : <div className="empty-state"><Boxes size={34} /><strong>Žádná komponenta neodpovídá filtrům</strong></div>}
    </section>

...
| Legacy API identity convergence | migration `088`, managed-service deploy invariant | every managed API/MCP compatibility record uses its bound component hostname and matching OAuth resource; tokens issued for the retired audience are revoked and the change is audited |
